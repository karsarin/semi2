...
public class ManagerMainViewServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 전체 목록 개수 조회함
		int qlistCount =  mservice.getQnAListCount();	
		// 해당 페이지용 목록 조회
		ArrayList<Question> qlist = mservice.selectQnAList(qcurrentPage, qlimit);
		// 총 페이지 수 계산 : 목록이 최소 1개일 때는 한 페이지로 처리함
		// 페이지 1이 되려면 = 목록 0.1 개 + 0.9 계산되게 함
		int qmaxPage = (int) ((double) qlistCount / qlimit + 0.9);
		// 현재 페이지가 13이면 화면에 보여줄 시작 페이지는 11로 지정
		// (1, 11, 21, 31, ........)
		int qstartPage = (((int) ((double) qcurrentPage / qlimit + 0.9)) - 1) * qlimit + 1;
		// 만약, 목록 아래에 보여질 페이지 개수가 10개이면
		// 현재 페이지가 13이면 끝 페이지수는 20페이지가 되어야 함
		int qendPage = qstartPage + qlimit - 1;
