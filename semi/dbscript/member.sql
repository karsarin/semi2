...
CREATE TABLE MEMBER(
  ...
  TALENT VARCHAR2(255),
  MGRCHAT      CHAR(1)  DEFAULT 'Y' CONSTRAINT NN_CHAT NOT NULL,
  MGRLOGIN     CHAR(1) DEFAULT 'Y' CONSTRAINT NN_LOGIN NOT NULL,
  CONSTRAINT CK_CHAT_LOGIN CHECK(MGRCHAT IN ('N','Y')), CHECK(MGRLOGIN IN ('N','Y'))
);
...
COMMENT ON COLUMN MEMBER.MGRCHAT IS '채팅가능여부';
COMMENT ON COLUMN MEMBER.MGRLOGIN IS '로그인가능여부';
